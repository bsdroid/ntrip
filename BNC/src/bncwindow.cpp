...
//pppLayout1->addItem(new QSpacerItem(4*ww, 0),              ir, 2);
  ...
  _rnxV2Priority->setWhatsThis(tr("<p>Specify a priority list of characters defining signal generation attributes as defined in RINEX Version 3. Priorities will be used to map observations with RINEX Version 3 attributes from incoming streams to Version 2. The underscore character '_' stands for undefined attributes. A question mark '?' can be used as wildcard which represents any one character.</p><p>Default is priority list 'CWPX_?'.</p>"));
  ...
  _serialAutoNMEAComboBox->setWhatsThis(tr("<p>Select 'Auto' to automatically forward NMEA messages of type GPGGA or GNGGA coming from your serial connected receiver to the NTRIP broadcaster and/or save them in a file.</p><p>Select 'Manual' if you want BNC to produce and forward one initial GPGGA NMEA message because your serial connected receiver doesn't generate these messages.</p>"));
  ...
  _serialHeightNMEALineEdit->setWhatsThis(tr("<p>Specify an approximate 'Height' above mean sea level in meters for the transmitting reference station introduced through 'Mountpoint'. Together with the latitude and longitude from the NTRIP broadcaster sourcetable the height information is used to build one initial GPGGA message to be sent to the NTRIP broadcaster.</p><p>The setting of this option is ignored if option 'NMEA' is set to 'Auto'.</p>"));
  ...
  _cmbUseGlonass->setWhatsThis(tr("<p>In case the incoming orbit and clock correction stream(s) support GLONASS, you can tick 'Use GLONASS' to produce a GPS plus GLONASS combination solution.</p><p>Default is a GPS-only combination."));
  ...
  _uploadAntexFile->setWhatsThis(tr("<p>As SP3 file contents should be referred to the satellites Center of Mass (CoM) while correctors are referred to the satellites Antenna Phase Center (APC), an offset has to be applied which is available from an IGS ANTEX file. You should therefore specify the 'ANTEX file' if you want to save the stream contents in SP3 format.</p><p>If you don't specify an 'ANTEX file' path, the SP3 file contents will be referred to the satellites APCs.</p>"));
