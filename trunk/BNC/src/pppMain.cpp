...
void t_pppMain::readOptions() {
  ...
  while (iSta.hasNext()) {
    ...
    opt->_aprSigCrd[0] = hlp[1].toDouble()+1e-300;
    opt->_aprSigCrd[1] = hlp[2].toDouble()+1e-300;
    opt->_aprSigCrd[2] = hlp[3].toDouble()+1e-300;
    opt->_noiseCrd[0]  = hlp[4].toDouble()+1e-300;
    opt->_noiseCrd[1]  = hlp[5].toDouble()+1e-300;
    opt->_noiseCrd[2]  = hlp[6].toDouble()+1e-300;
